function CNTable(oParentContainer, oCpuDifficultyParams, playerId) {
  ...
  // Добавляем трекинг времени последнего удара
  var _iLastShotTime = 0;

  this.getLastShotTime = function () {
    return _iLastShotTime;
  };

  this.isReadyForShot = function () {
    return _bReadyForShot && !_bHoldStick && !_bForceStopStick;
  };

  this.playerShot = function () {
    if (Date.now() - _iLastShotTime < 500) {
      return;
    }
  ...
  this.syncGameState = function (gameState) {
    ...
    _bReadyForShot = gameState.readyForShot;
    _bHoldStick = gameState.holdStick;
    _bForceStopStick = gameState.forceStopStick;

    // Синхронизируем другие важные состояния
    _iState = gameState.state;
  ...
  this.changeTableStyle = function(spriteKey) {
    ...
    console.log("Обновляем стиль стола на:", spriteKey);
    
    // Сохраняем текущую позицию и масштаб
    var currentX = _oContainer.x;
    var currentY = _oContainer.y;
    
    // Заменяем спрайт стола (обычно это первый элемент в контейнере)
    if (_oContainer.children && _oContainer.children.length > 0) {
      _oContainer.removeChildAt(0);
      var oBg = createBitmap(oSprite);
      _oContainer.addChildAt(oBg, 0);
      
      // Восстанавливаем позицию
      _oContainer.x = currentX;
      _oContainer.y = currentY;
      
      // Обновляем линии траектории, чтобы они были поверх стола
      if (_oDollyDir) _oContainer.setChildIndex(_oDollyDir, _oContainer.children.length-1);
      if (_oCueBallDir) _oContainer.setChildIndex(_oCueBallDir, _oContainer.children.length-1);
      if (_oHittenBallDir) _oContainer.setChildIndex(_oHittenBallDir, _oContainer.children.length-1);
      
      console.log("Стиль стола успешно обновлен");
      return true;
    }
    
    console.warn("Не удалось найти спрайт стола в контейнере");
    return false;
  };
  ...
  // Добавляем новые методы для работы с кием
  this.getStick = function() {
    return _oStick;
  };
  
  this.changeCueStyle = function(spriteKey) {
    if (_oStick && typeof _oStick.changeStyle === 'function') {
      _oStick.changeStyle(spriteKey);
      return true;
    }
    return false;
  };
