function CTable(oParentContainer, oCpuDifficultyParams) {
  ...
  this._init = function (oCpuDifficultyParams) {
    ...
    const userTableStyle = localStorage.getItem('selectedTable') || 'default';
    const userCueStyle = localStorage.getItem('selectedCue') || 'default';

    var oSpriteBg = s_oSpriteLibrary.getSprite(
      userTableStyle === 'default' ? "pool_table" :
      userTableStyle === 'red' ? "pool_table_red" : 
      userTableStyle === 'blue' ? "pool_table_blue" : "pool_table"
    );
  ...
  this.changeTableStyle = function(spriteKey) {
    ...
    if (oSprite) {
        // Сохраняем текущую позицию и размеры стола
        var currentX = _oContainer.x;
        var currentY = _oContainer.y;
        
        // Обновляем спрайт стола
        ...
        localStorage.setItem('selectedTable', spriteKey);
    }
};
...
this.changeCueStyle = function(spriteKey) {
    if (_oStick) {
        ...
        // Сохраняем выбранный стиль кия в localStorage
        localStorage.setItem('selectedCue', spriteKey);
    }
};
