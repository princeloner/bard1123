function CProfile() {
  ...
  this.onJoinRes = function (res) {
    console.log('[CLIENT] Получен joinroom-res', res);
    if (res.success) {
      if (window.s_oRoomList && typeof window.s_oRoomList.onOpenRoom === "function") {
        window.s_oRoomList.onOpenRoom(res.pid);
        console.log('[CLIENT] onOpenRoom вызван из onJoinRes, pid:', res.pid);
      }
  ...
  this.onOpenRoom = function (pid) {
    console.log('[CLIENT] onOpenRoom вызван, pid:', pid);
    this._onExit(function () {
      console.log('[CLIENT] _onExit завершён, вызываем unload');
      s_oRoomList.unload();
      console.log('[CLIENT] unload завершён, вызываем gotoGame');
      if (typeof s_oMain.gotoGame === 'function') {
        s_oMain.gotoGame(pid);
        console.log('[CLIENT] gotoGame вызван с pid:', pid);
      } else {
        console.error('[CLIENT] s_oMain.gotoGame не определён!');
      }
      $(s_oMain).trigger("start_session");
      console.log('[CLIENT] start_session trigger');
    });
  };
  ...
  this.joinRoom = function (roomid) {
    console.log('[CLIENT] Клик по join для комнаты', roomid);
    socket.emit("joinroom-req", roomid);
    console.log('[CLIENT] Отправлен socket.emit("joinroom-req", roomid)', roomid);
  };
  ...
  this.onCreateRoom = function (res) {
    console.log('[CLIENT] Получен ответ createroom-res:', res);
    if (res.success) {
      if (window.s_oRoomList && typeof window.s_oRoomList.onOpenRoom === "function") {
        console.log('[CLIENT] onOpenRoom вызван из onCreateRoom, pid:', res.pid || socket.id);
        window.s_oRoomList.onOpenRoom(res.pid || socket.id);
      } else {
        console.error('[CLIENT] window.s_oRoomList.onOpenRoom не определён!');
      }
  ...
  this.openRoom = function () {
    console.log('[CLIENT] Клик по созданию комнаты (openRoom)');
    var amount = $("#bet_amount").val();
    var isPrivate = document.getElementById("permission").checked;
    console.log('[CLIENT] Значения для создания комнаты:', {amount, isPrivate});
    socket.emit("createroom-req", {
      amount,
      isPrivate,
    });
    console.log('[CLIENT] Отправлен socket.emit("createroom-req")', {amount, isPrivate});
  };
